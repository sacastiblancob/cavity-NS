function [WH,b] = csr_arnoldi_householder(Av,Ac,Ar,v,m,LUv,pc)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%              CSR_packed storage, i.e., if pc==3 or pc==4
...
for j=1:m+1
    ...
    if j<=m  %this if presumibly can be removed, if Q dimensions are (n,m+1)
        ...
        % z_j+1 = Pj*Pj-1*...*P2*P1*(A*v_j)
        % i.e succesive outer products by left side
        zaux = csr_solpacklu(LUv, Ac, Ar, v);
        zaux = csr_matvec(Av,Ac,Ar,zaux);
        for k = 1:j
            bzTw = b(k)*(zaux(k:n)'*WH(k+1:n+1,k));
            for i = k:n
