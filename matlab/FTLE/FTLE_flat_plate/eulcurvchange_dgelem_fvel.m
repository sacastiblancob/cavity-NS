function [K1,K0,dxdep,dydep,XDGgt1,YDGgt1] = ...
    eulcurvchange_dgelem_fvel(ep,et,XDGgt0,YDGgt0,dt,NT,NT1,Nx,Ny,XDD,YDD)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This function computes de Eulerian Curvature Change (kt0) with DG operators
%
% Inputs:
...
for t=1:NT
    [XDGgt0,YDGgt0] = AB3_fvel(XDGgt0,YDGgt0,dt,0);
end

%Initial Curvature Scalars
[dxdep,dydep] = derv_epet_dg(ep,et,XDGgt0,YDGgt0,XDD,YDD);
...
%AB3 for particle tracing from parallel material lines to t0
% and Euler for solving material curvature rate equation
XDGgt1 = XDGgt0;
YDGgt1 = YDGgt0;
...
for t=1:NT1
    ...
    for j=1:Nx+1
        for i=1:Ny+1
            nepet = norm([dxdep(i,j) dydep(i,j)]);
            dxdep(i,j) = dxdep(i,j)/nepet;
            dydep(i,j) = dydep(i,j)/nepet;
        end
    end
    
    %Computing velocity field
    [UDG1,VDG1] = f_vel(XDGgt1,YDGgt1,0);
    ...
    %Solving Material Curvature Rate Eq. (incompressible) with Forward-Euler
    rhs = rns - nor - K1.*(3*rs);
    K1 = K1 + dt*rhs;

    %Advecting particles
    [XDGgt1,YDGgt1] = AB3_fvel(XDGgt1,YDGgt1,dt,0);
